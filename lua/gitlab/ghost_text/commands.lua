...
M.insert_word          = function()
    ...
    local word_start, word_end = text:find("(%s*[a-zA-Z0-9_]+)")
    local non_word_start, non_word_end = text:find("(%s*[^a-zA-Z0-9_]+)")
    local start_col = math.min(word_start or math.huge, non_word_start or math.huge)
    local end_col = math.min(word_end or math.huge, non_word_end or math.huge)
    if start_col == math.huge or end_col == math.huge then
        return
    end

    local partial = text:sub(start_col, end_col)
    local remainder = text:sub(end_col + 1)
